  ...
  return Result.tryPromise({
    try: async () => {
      await new REST()
        ...
        .post(Routes.channelMessages(job.channel as string), {
          body: {
    ...
    catch: (cause) => new DiscordError({ operation: "notify", cause }),
  });
};

export const notifyNewProduct = (product: Product, search: TableRow) => {
  const message = new TextDisplayBuilder().setContent(
